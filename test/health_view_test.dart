...
MonitoringData _data({
  List<MemberEventEntry>? events,
  List<SlowRequestEntry>? slow,
}) => MonitoringData(
  memberEvents: events ?? [],
  slowRequests: slow ?? [],
...
void main() {
  ...
  testWidgets('renders a failed sign-in with member and club named', (
    tester,
  ) async {
    await setDesktopSize(tester);
  ...
  testWidgets('an unmatched identifier is shown as Unknown, not dropped', (
    tester,
  ) async {
    await setDesktopSize(tester);
    final state = DashboardState()
